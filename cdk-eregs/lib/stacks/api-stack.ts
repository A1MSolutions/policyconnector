...
class SecurityGroupHandler {
  ...
  static createOrImportSecurityGroup(
    ...
    // Add inbound rules
    serverlessSG.addIngressRule(ec2.Peer.anyIpv4(), ec2.Port.tcp(443), 'Allow HTTPS inbound');
...
export class BackendStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props: BackendStackProps, stageConfig: StageConfig) {
    ...
    // Add this near the start of your stack, before creating any API Gateways
    ...
    try {
      secrets.oidc = secretsmanager.Secret.fromSecretNameV2(
        this,
        'OidcSecret',
        SECRETS.OIDC_CREDENTIALS
      );
      console.log('Successfully loaded OIDC credentials secret');
    } catch (error) {
      console.error('Failed to load OIDC credentials secret:', error);
      throw error;
    }

    // Load Django admin credentials
    try {
      secrets.django = secretsmanager.Secret.fromSecretNameV2(
        this,
        'DjangoSecret',
        SECRETS.DJANGO_CREDENTIALS
      );
      console.log('Successfully loaded Django admin credentials secret');
    } catch (error) {
      console.error('Failed to load Django admin credentials secret:', error);
      throw error;
    }

    // Load Reader credentials
    try {
      secrets.reader = secretsmanager.Secret.fromSecretNameV2(
        this,
        'ReaderSecret',
        SECRETS.READER_CREDENTIALS
      );
      console.log('Successfully loaded Reader credentials secret');
    } catch (error) {
      console.error('Failed to load Reader credentials secret:', error);
      throw error;
    }

    // Load DB credentials
    try {
      secrets.db = secretsmanager.Secret.fromSecretNameV2(this, 'DbSecret', SECRETS.DB_CREDENTIALS);
      console.log('Successfully loaded DB credentials secret');
    } catch (error) {
      console.error('Failed to load DB credentials secret:', error);
      throw error;
    }

    // Load HTTP credentials
    try {
      secrets.http = secretsmanager.Secret.fromSecretNameV2(
        this,
        'HttpSecret',
        SECRETS.HTTP_CREDENTIALS
      );
      console.log('Successfully loaded HTTP credentials secret');
    } catch (error) {
      console.error('Failed to load HTTP credentials secret:', error);
      throw error;
    }
    ...
    // ================================
    const buildId =
      this.node.tryGetContext('buildId') || process.env.RUN_ID || new Date().getTime().toString();
